...
```{r Reload, include=T}
#Used the data from the dataset for ethnicity to help with creating the results for gender.
...
We are comparing the gender and the blood pressure data from a total of `r sum(Gender_Count)` participants who passed the data preprocessing steps described above (i.e., those that are about the age of 17 and contained adequeate blood pressure data).  Of those accounted for, there are:
...
Just like with the histograms for the various ethnicities, the shape of the histograms (which are generally more positively skewed and narrow than a normal distribution curve for the systolic and diastolic pressure distributions, respectively) and the Shapiro-Wilk Test probability values from each, only the diastolic pressures for both males and females should be considered normally-distributed.
...
```{r TTest, include=T}
ttest_Syst = t.test(Female_test$SystolAvg, Male_test$SystolAvg)
ttest_Syst

ttest_Dias = t.test(Female_test$DiastolAvg, Male_test$DiastolAvg)
ttest_Dias
```

The Systol average for male and female are `r format(ttest_Syst$estimate)` respectively. The two-sample t-test shows a small p-value of `r format(ttest_Syst$p.value)`, thereby rejected the null, and adopting the alternate hypothesis of the two groups have different means at $\alpha$ = 0.01 (or even 0.001).

The Diabolic average for male and female are `r format(ttest_Dias$estimate)` respectively. The two-sample t-test shows a small p-value of `r format(ttest_Dias$p.value)`, thereby rejected the null, and adopting the alternate hypothesis of the two groups have different means at $\alpha$ = 0.01 (or even 0.001).
...
A $\chi^2$-test for Female's blood pressure confirms the p-value to be `r format(chitest$p.value)`. So we can adopt that the two frequency distributions are not the same. In other words, this is equivalent to saying that Systostic and Diastolic are not independent of each other. There is an effect on females from the different blood pressures.
...
Just like with the histograms for the various ethnicities, the shape of the histograms (which are generally more positively skewed and narrow than a normal distribution curve for the systolic and diastolic pressure distributions, respectively) and the Shapiro-Wilk Test probability values from each, only the diastolic pressures for both veterans and non-veterans should be considered normally-distributed.
...
ttest_Vet_Syst = t.test(Veterans_test$SystolAvg, Non_Veterans_test$SystolAvg)
ttest_Vet_Syst

ttest_Vet_Dias = t.test(Veterans_test$DiastolAvg, Non_Veterans_test$DiastolAvg)
ttest_Vet_Dias
```


The Systol average for veterans and non veterans are `r format(ttest_Vet_Syst$estimate)` respectively. The two-sample t-test shows a small p-value of `r format(ttest_Vet_Syst$p.value)`, thereby rejected the null, and adopting the alternate hypothesis of the two groups have different means at $\alpha$ = 0.01 (or even 0.001).

The Diabolic average for male and female are `r format(ttest_Vet_Dias$estimate)` respectively. The two-sample t-test shows a small p-value of `r format(ttest_Vet_Dias$p.value)`, thereby rejected the null, and adopting the alternate hypothesis of the two groups have different means at $\alpha$ = 0.01 (or even 0.001).
