...
After that, we do some of the feature selection and extraction. First make subsets to narrow rows by age (age>17) and select relevant columns (ethnicity, gender, annual household, age, veteran status). Second, remove all missing values or put into expected values. Next, create two new columns that average the values from all the blood pressure data for systolic and diastolic blood pressures (except for NAs). Later we will use the two average for all the further evaluations. Then, drop all missing values with rows with 0 or NA in the Average Systolic and Diastolic columns, also drop all rows that we don’t need (irrelevant with our topics/the original stimulant/depressant marker questions…). The summary of database meet all the needs now.
...
The systolic average for veterans are `r format(ttest_Vet$estimate)` respectively. The two-sample t-test shows a small p-value of `r format(ttest_Vet$p.value)`, thereby rejected the null with the means at $\alpha$ = 0.05 (or even 0.005).

The diastolic average for non veterans are `r format(ttest_NonVet$estimate)` respectively. The two-sample t-test shows a small p-value of `r format(ttest_NonVet$p.value)`, thereby rejected the null with the means at $\alpha$ = 0.05 (or even 0.005).
...
```                                                                                      
Use dataframe function to make all the varibales numerical before doing the correlation test.
# Correction test
...
```                                                                                       
From the corrplot above, we can easily see that for younger and middle-aged group, there is no big correalation between age and blood pressure neither for systolic average blood pressure nor dialstolic average blood pressure. For older people, age does have an effect on blood pressure. When you get older, your blood pressure tends to become higher than before which means that you are less healthy.
...
I divide the blood pressure into two groups as before, which are systolic average blood pressure and diastolic blood pressure. In this chunk, we mainly discuss age groups whether have an effect on systolic blood pressure. The model is simple as we can see because we only have one varibale inside. The middle-aged people will have 15.058 smaller systolic average blood pressure than the old. Yonger people, in the meantime expected to have 20.405 smaller systolic average blood pressure. This is not a model to some extent. I would rather call this some descriptive statistics. I also make some plots that can easily explain the relationship between two varibales.
...
### Mean of Systolic and Diastolic Blood Pressure Average
